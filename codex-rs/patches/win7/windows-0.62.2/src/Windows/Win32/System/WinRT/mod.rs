...
where
    T: windows_core::Interface,
{
    // Win7 兼容性：RoGetActivationFactory 在 Win7 上不存在，通过 GetProcAddress 动态加载
    ...
    static FN: AtomicPtr<core::ffi::c_void> = AtomicPtr::new(core::ptr::null_mut());
    let fn_ptr = FN.load(Ordering::Acquire);
    let proc = if fn_ptr.is_null() {
        let mut ole32_buf = [0u16; 14];
        ole32_buf[..8].copy_from_slice(&['o' as u16, 'l', 'e', '3', '2', '.', 'd', 'l', 'l', '\0' as u16]);
        let module = windows_sys::Win32::System::LibraryLoader::GetModuleHandleW(ole32_buf.as_ptr());
        if module == 0 {
            return Err(windows_core::Error::from_hresult(windows_core::HRESULT(0x80040001_u32 as _)));
        }
        let mut name_buf = [0i8; 24];
        let name = b"RoGetActivationFactory\0";
        for (i, &c) in name.iter().enumerate() {
            name_buf[i] = c as i8;
        }
        let p = windows_sys::Win32::System::LibraryLoader::GetProcAddress(module, name_buf.as_ptr());
        if p.is_null() {
            return Err(windows_core::Error::from_hresult(windows_core::HRESULT(0x80040001_u32 as _)));
        }
        FN.store(p as *mut _, Ordering::Release);
        p
    } else {
        fn_ptr
    };
    let mut result__ = core::ptr::null_mut();
    let hr = unsafe {
        let f: FnRoGetActivationFactory = core::mem::transmute(proc);
        f(core::mem::transmute_copy(activatableclassid), &T::IID, &mut result__)
    };
    windows_core::Error::from_hresult(hr).and_then(|_| unsafe { windows_core::Type::from_abi(result__) })
}
#[inline]
