...
//!
//! The history is stored at `~/.gienx/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.gienx`.
...
pub async fn append_entry(
    ...
    // Resolve `~/.gienx/history.jsonl` and ensure the parent directory exists.
